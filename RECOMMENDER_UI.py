...
# HIỂN THỊ NỘI DUNG ĐỀ XUẤT KHÁCH SẠN THEO COSINE SIMILARITY
    ...
    for i,image_path in enumerate(list_path_rec_hotel_pic):
        ...
        with st.expander(label=hotel_name,expanded=True):
...
# HIỂN THỊ NỘI DUNG ĐỀ XUẤT KHÁCH SẠN THEO ALS
    ...
    name=st.text_input(label='Tên đăng nhập',
                       placeholder='Nhập tên đăng nhập tại đây',
                       help='Lưu ý tên đăng nhập có 1 chữ').title()
    nationality=st.text_input(label='Quôc tịch',
                              placeholder='Nhập quốc tịch tại đây',
                              help='Lưu ý ghi đúng tên quốc tịch theo tiếng việt').title()
    ...
    if name and nationality and password=='1234' and button:
        ...
        with image_section:
            st.image('avata.jpg',caption='Ảnh đại diện')
        st.subheader('CÓ THỂ BẠN QUAN TÂM')
        df_recommended_hotel=ut.Collaborative_filtering_recommender_system(name,nationality,8)
        list_path_rec_hotel_pic=glob.glob('rec_hotel_?.jpg')
        for i,image_path in enumerate(list_path_rec_hotel_pic):
            hotel_name=df_recommended_hotel.iloc[i,2]
